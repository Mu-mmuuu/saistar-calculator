...
# def calculator(song_name,appeal,idol,Pskill_fullname,Pskill_Lv,trial):
    ...
    skill_fullname = idol_data[:,7]
    interval = idol_data[:,9]
    skill_percent = idol_data[:,10]/100
    skill_rate = 1+idol_data[:,11]/100
    ...
    np.place(skill_name,skill_name == 'ダブルブースト','dobble')
    ...
    for i in range(6):
        interval_judge = np.where(score_data[0] % int(interval[i]) < math.ceil(int(interval[i])/2)) 
    ...
    def calculator(skill_percent):
                ...
                for  t in Pskill_timing:
                    for n in range(np.max(skill_maxact)):
                        if skilltiming[i][n] == 0:
                            continue
                        if skilltiming[i][n] - t < Pskill_time and skilltiming[i][n] - t >= 0:
                            random_result[i][n] = random.choices([1.0,skill_rate[i]*float(Pskill_value)],weights = [1-skill_percent[i],skill_percent[i]],k=1)[0]                             
                ...
                for  t in Pskill_timing:
                    for n in range(np.max(skill_maxact)):
                        if skilltiming[i][n] == 0:
                            continue
                        if skilltiming[i][n] - t < Pskill_time and skilltiming[i][n] - t >= 0:
                            random_result[i][n] = random.choices([1.0,skill_rate[i]],weights = [1-(skill_percent[i]*float(Pskill_value)),skill_percent[i]*float(Pskill_value)],k=1)[0]                             
        ...
        dobble_skillrate = skill_calc[np.where(skill_name == 'dobble')]
        dobble_skillrate -= 1
        ...
        if len(dobble_skillrate) == 0:
            dobble_skillrate = np.zeros((1,notes))

        score_skillrate = np.max(score_skillrate,axis=0)
        combo_skillrate = np.max(combo_skillrate,axis=0)
        dobble_skillrate = np.max(dobble_skillrate,axis=0)
        score_skillrate += dobble_skillrate
        combo_skillrate += dobble_skillrate
